...
class EmulatorFileSystem {
  ...
  async prepare() {
    ...
    if (typeof this.emscriptenModule.monitorRunDependencies === "function") {
      await this.emscriptenModule.monitorRunDependencies();
    }
  }
}
...
`);
  ...
  runMain() {
    ...
    const { Module } = this.getEmscripten();
    const raArgs = [...this.initialArgs];
    const { rom, signal } = this.options;
    if (this.initialArgs.length === 0 && rom.length > 0) {
      const [{ name }] = rom;
  ...
  async setupEmscripten() {
    ...
    if (Array.isArray(moduleOptions.arguments)) {
      this.initialArgs = [...moduleOptions.arguments];
    }
  ...
  updateKeyboardEventHandlers() {
    const { JSEvents } = this.getEmscripten();
    if (!JSEvents?.eventHandlers || !JSEvents?.registerOrRemoveHandler) {
      return;
    }
